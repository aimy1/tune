...
pub fn draw_loading(frame: &mut Frame, app: &App) {
    ...
    // Subtle status under the bar when there is room.
    if areas[1].height >= 3 {
        let status_y = bar_area.y.saturating_add(1);
        if status_y < areas[1].y + areas[1].height {
            let pct = (progress * 100.0).round() as u16;
            let status = format!("loading · {pct}%");
            frame.render_widget(
