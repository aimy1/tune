...
mod tests {
    ...
    fn test_available_themes_contains_builtins() {
        ...
        assert!(themes.contains(&"noctalia".to_string()));
    ...
    #[test]
    fn test_load_noctalia_theme() {
        let theme = ThemeLoader::load_with_overrides("noctalia", None).unwrap();
        assert_eq!(theme.name, ThemeName::Noctalia);
        // Ensure valid non-black text color
        assert!(theme.palette.text.0 > 0 || theme.palette.text.1 > 0 || theme.palette.text.2 > 0);
    }
