...
<p align="center">
  ...
  <a href="https://github.com/aimy1/tune/releases"><img src="https://img.shields.io/badge/Release-v0.7.2-success.svg?style=flat-square" alt="Release v0.7.2" /></a>
...
`tune` 是一款专为终端极客与音乐爱好者打造的网易云音乐终端播放器。基于 **Rust** 与 **Ratatui** 异步渲染引擎构建，追求极致轻量、亚毫秒级输入响应，融入 **Catppuccin** 全色系与 **Noctalia** 动态壁纸配色、**全终端原生图像协议（Auto / Kitty / Sixel / iTerm2）**、**1:1 无畸变高清专辑封面与频谱可视化**、**跨屏拖拽悬浮桌面歌词**、**集中化透明度调控** 以及沉浸式全屏微动效与鼠标操作，让终端听歌体验兼具极客速度与现代美学。
...
| 模块 | 特性说明 |
| :--- | :--- |
| 🎨 **设计美学 & 动态配色** | 原生搭载 **Catppuccin** 官方四色系（Mocha、Macchiato、Frappe、Latte）、Hyprland 配色以及 **Noctalia** 动态壁纸配色（支持从系统壁纸自动提取 Material You 主色）；支持 `[palette]` 十六进制色值精确覆盖与免重启热重载。 |
| 🪟 **透明设置专区** | 全新集中化透明度调控中心（快捷键 <kbd>,</kbd> 进入设置第 2 项），一站式配置 **背景透明**、**侧边栏透明**（同时覆盖标准模式与全屏模式侧边栏，透出精美终端壁纸）以及**桌面歌词背景样式与透明度**。 |
| 🖼️ **高清封面 & 多协议** | 基于 `ratatui-image` 原生支持 **Auto / Kitty / Sixel / iTerm2 / Halfblocks** 多图像协议免重启热切换；重构全屏专辑封面渲染几何算法，无论是否开启边框均保持 **严格 1:1 物理视觉正方形**，杜绝拉伸与压缩。 |
| 🖱️ **全局鼠标交互** | 提供全局可控的鼠标支持（可在设置中随时开启/关闭），支持选项点击切换/进入、外部点击关闭返回、滚轮流畅上下滚动；底部控制栏支持鼠标直接点击播放/暂停、快进快退、音量调节胶囊及桌面歌词 `[词]` 开关。 |
| 🎛️ **全屏沉浸视窗** | 双栏沉浸式播放视窗；十段图形均衡器（EQ）；跳动音频频谱；**单行极简悬浮音量胶囊**（不遮挡进度条，支持滚轮/点击微调）；右侧平滑推拉播放队列；设置与关于弹窗在全屏下与主程序无缝对齐。 |
| 🪟 **独立桌面歌词** | 独立的现代化透明置顶悬浮歌词客户端（支持 Wayland Layer Shell / X11）；支持跨屏幕平滑拖动与实时坐标回显、拖拽高亮边框；支持宽度、透明度、字号、单双行、对齐方式等多维度个性化定制。 |
...
- **全局极速联动**：随时按快捷键 <kbd>d</kbd> 即可一键显示/隐藏桌面歌词，按 <kbd>Alt+S</kbd> 随时切换锁定 / 拖动开关；底部控制栏右侧也常驻可点击的 `[词]` 开关按钮。
...
| <kbd>,</kbd> | 打开系统设置与快捷键偏好菜单（支持 Vim 键位 `h`/`j`/`k`/`l` 导航、`Backspace` 返回，支持鼠标点击与滚轮） |
| <kbd>b</kbd> | 展开 / 折叠主页侧边栏 |
| <kbd>f</kbd> | 进入 / 退出沉浸式全屏播放模式 |
| <kbd>d</kbd> | 快捷开启 / 关闭悬浮桌面歌词（控制栏右侧亦有 `[词]` 按钮可点击） |
| <kbd>Alt+S</kbd> | 快捷锁定 / 解锁悬浮桌面歌词（开启/关闭拖动模式） |
| <kbd>Ctrl+L</kbd> | 强制重绘并刷新终端屏幕（清除底层 ALSA/驱动偶发的输出扰动） |
...
| 按键 | 功能描述 |
| :---: | :--- |
| <kbd>[</kbd> / <kbd>]</kbd> | 播放上一首 / 下一首曲目（标准模式与全屏模式通用） |
| <kbd>Space</kbd> | 播放 / 暂停当前曲目（控制栏图标可鼠标点击） |
| <kbd>Enter</kbd> | 播放选中歌曲、进入歌单、确认选中项 |
| <kbd>Esc</kbd> | 返回上一级 / 关闭当前弹出浮层（弹窗外侧亦可鼠标点击关闭） |
| <kbd>l</kbd> | 喜欢 / 取消喜欢当前播放歌曲（红心联动，标准与全屏模式通用） |
| <kbd>←</kbd> / <kbd>→</kbd> | 全屏模式后退 / 快进 5 秒；标准模式下切换卡片焦点（标准模式亦支持 <kbd>Shift+←</kbd> / <kbd>Shift+→</kbd> 微调进度） |
| <kbd>↑</kbd> / <kbd>↓</kbd> | 在曲目列表、推荐卡片或菜单间移动焦点（支持鼠标滚轮） |
| <kbd>Ctrl+↑</kbd> / <kbd>Ctrl+↓</kbd> | 侧边栏“创建歌单”与“收藏歌单”分区快捷跳转 |

### 全屏模式专属控制
| 按键 | 功能描述 |
| :---: | :--- |
| <kbd>t</kbd> | 快速打开 / 关闭全屏设置面板（含透明设置二级菜单、图像协议热切换等全量配置） |
| <kbd>e</kbd> | 打开 / 关闭十段图形均衡器 (EQ) 调节窗口 |
| <kbd>v</kbd> | 呼出 / 关闭单行极简音量调节胶囊条（支持滚轮/方向键微调） |
| <kbd>p</kbd> | 展开 / 关闭右侧播放队列列表（支持 Vim 键位 `j`/`k` 导航，按 `Esc` 退出列表） |
...
- **预设主题**：`~/.config/tune/themes/`
  - `noctalia.toml`（**推荐**：从系统壁纸动态提取配色，Material You 风格）
...
```toml
# 主题设置：支持内置预设 ("noctalia", "mocha", "hyprland" 等) 或自定义主题文件路径
theme = "noctalia"

# 透明度与外观
transparent_background = true   # 背景透明
transparent_sidebar = true      # 侧边栏透明 (同时生效于主界面与全屏侧边栏)
album_border = true             # 全屏模式封面边框 (无论开关均保持严格 1:1 正方形)

# 图像协议：auto / kitty / sixel / iterm2 / halfblocks / off (支持免重启热切换)
graphics_protocol = "auto"

# 鼠标交互：true / false (支持鼠标点击、滚轮翻页、外部点击关闭)
mouse_support = true

# 可选：自定义调色板覆盖 (覆盖当前 theme 的对应配色)
# [palette]
# text = "#F2F4F8"
# accent = "#33CCFF"
```

所有关于桌面歌词、快捷键绑定、播放记忆与缓存清理等偏好设置均会自动持久化保存于此。
